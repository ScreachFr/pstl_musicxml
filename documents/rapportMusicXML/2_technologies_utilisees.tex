...
\largemusicsize % grande taille pour toute les partitions
...
Relax NG (\textbf{Re}gular \textbf{La}nguage for \textbf{X}ML \textbf{N}ext \textbf{G}eneration) \cite{relaxng} est né de la fusion de TreX de James Clark et de Relax de Murata Makoto. C'est un langage qui permet de définir la grammaire d'un document XML. Relax NG ne s'intéresse qu'à la structure du document et non à sa valeur.

\par
Relax NG s'utilise assez facilement. Il faut tout d'abord écrire la grammaire du document \emph{XML} que nous souhaitons définir. Pour ce programme nous utilisons une grammaire écrite par \cite{relaxng_for_musicxml}. Nous devons ensuite la compiler et l'utiliser pour valider des documents grâce aux outils fourni par \cite{relaxng}.
...
\textbf{Trang} \cite{trang} et \textbf{Jing} \cite{jing} sont deux librairies développées par Thai Open Source. Elles permettent de générer des grammaires Relax NG et de valider des documents XML à partir de cette même grammaire.

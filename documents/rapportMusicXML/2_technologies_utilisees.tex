...
OpenMusic est un langage de programmation visuel basé sur le langage Lisp qui permet d'écrire des programmes graphiquement. Il a été conçu par les chercheurs de l'IRCAM Carlos Agon, Gérard Assayag et Jean Bresson.
...
\par
Les images suivantes sont des exemples d'arbres rythmiques.
...
Une fois le fichier XML parsé, nous disposons d'un \emph{DOM} sous forme d'un objet \emph{document}. Pour afficher ce document il nous suffit d'appeler récursivement un méthode qui affiche un nœud de l'arbre sur la racine du \emph{Document}. Cette méthode s'appelle récursivement sur les fils du nœud courant. Cela nous permet donc d'afficher tout l'arbre.
...
\textbf{Trang} et \textbf{Jing} sont deux librairies développées par \textbf{Thai Open Source}. Elles permettant de générer des grammaires Relax NG et de valider des documents XML à partir de cette même grammaire.
...
SAX est une API créée par David Megginson en 1998 et est l'acronyme de \textbf{S}imple \textbf{A}PI for \textbf{X}ML. Elle permet de manipuler des documents XML en utilisant des événements envoyées à chaque rencontre d'un élément.


\subsubsection{Le DOM}

Le DOM, ou \textbf{D}ocument \textbf{O}bject \textbf{M}odel, est une interface de programmation normalisée par le W3C et est indépendant de tout plateforme et langage. Il voit les documents à balises comme des arbres dont le contenu et la structure peuvent être accédés et mis à jour dynamiquement.
